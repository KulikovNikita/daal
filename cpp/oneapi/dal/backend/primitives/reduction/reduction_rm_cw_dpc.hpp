...
template <typename Float, typename BinaryOp, typename UnaryOp>
class reduction_rm_cw_super_accum_narrow {
    static_assert(std::is_same_v<Float, float>);
    static_assert(std::is_same_v<BinaryOp, sum<float>>);

public:
    constexpr static inline int max_folding = 128;
    constexpr static inline int block_size = 32;
    reduction_rm_cw_super_accum_narrow(sycl::queue& q);
    sycl::event operator()(const Float* input,
                           Float* output,
                           std::int64_t width,
                           std::int64_t stride,
...
public:
    ...
    using sacc_narrow_t = reduction_rm_cw_super_accum_narrow<Float, BinaryOp, UnaryOp>;

    reduction_rm_cw(sycl::queue& q);
    enum reduction_method { naive, naive_local, super_accum_wide, super_accum_narrow };
